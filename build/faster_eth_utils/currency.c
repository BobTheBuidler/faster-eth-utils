...
PyInit_currency(void)
{
    ...
    PyObject *capsule = PyObject_GetAttrString(tmp, "init_faster_eth_utils___currency");
    Py_DECREF(tmp);
    if (capsule == NULL) return NULL;
    void *init_func = PyCapsule_GetPointer(capsule, "faster_eth_utils__mypyc.init_faster_eth_utils___currency");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
...
PyMODINIT_FUNC PyInit___init__(void) { return PyInit_currency(); }
