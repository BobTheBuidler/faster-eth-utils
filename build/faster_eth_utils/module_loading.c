...
PyInit_module_loading(void)
{
    ...
    tmp = PyImport_ImportModuleLevel("faster_eth_utils__mypyc", NULL, NULL, fromlist, 0);
    if (!tmp) return NULL;
    ...
    if (deps_capsule != NULL) {
        int (*deps_func)(void) = (int (*)(void))PyCapsule_GetPointer(
            deps_capsule, "faster_eth_utils__mypyc.ensure_deps");
    ...
    PyObject *capsule = PyObject_GetAttrString(tmp, "init_faster_eth_utils___module_loading");
    Py_DECREF(tmp);
    if (capsule == NULL) return NULL;
    void *init_func = PyCapsule_GetPointer(capsule, "faster_eth_utils__mypyc.init_faster_eth_utils___module_loading");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
...
PyMODINIT_FUNC PyInit___init__(void) { return PyInit_module_loading(); }
