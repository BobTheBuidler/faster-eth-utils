...
split by submodule (benchmark file).
For each submodule, and for each group (e.g., "abi_to_signature"), finds both implementations
...
def compare_submodule(submodule_results: Dict[str, Any]) -> Dict[str, Any]:
    # Group by group name within submodule
    groups: Dict[str, Dict[str, Any]] = defaultdict(dict)
    for test_name, data in submodule_results.items():
...
def main() -> None:
    ...
    # results: {submodule: {test_func: {...}}}
    diff_by_submodule = {}
    for submodule, submodule_results in results.items():
        diff_by_submodule[submodule] = compare_submodule(submodule_results)
